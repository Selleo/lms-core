      ...
      <div className="flex flex-col items-center gap-y-2">
        <hgroup className="text-center">
          <h2 className="font-bold text-lg text-neutral-950">
            {isSubmitted
              ? "Your Latest Attempt"
              : "You haven’t finished this quiz yet."}
          </h2>
          {!isSubmitted ? (
            <p className="body-base">
              Once you complete the quiz, your score will appear here. You’ll
              also have the option to retake it anytime to improve your results.
            </p>
          ) : (
        ...
        {isSubmitted && (
          <div className="flex flex-col gap-y-3">
            <Button
              onClick={() =>
                clearQuizProgress.mutate({ lessonId: lessonId ?? "" })
              }
            >
              Try Again
            </Button>
            <span className="text-center text-neutral-800 body-sm">
              Clicking Try Again will erase your latest score. This action
              cannot be undone.
            </span>
          </div>
        )}
