  ...
  return (
    <div className="flex flex-col gap-y-8">
      <div className="flex gap-x-4 items-center">
        <p className="h2 text-neutral-950">Welcome back, {user?.firstName}</p>
        <Avatar className="size-12">
          <Gravatar email={user?.email} />
        </Avatar>
      </div>
      <div className="flex gap-x-7 h-[calc(100dvh-129px)]">
        <div className="w-full h-full gap-x-4 gap-y-6 flex flex-col">
          <div className="flex gap-x-4 w-full h-full">
            <div className="w-full h-full p-8 gap-y-4 bg-white rounded-lg drop-shadow-card max-w-[296px] flex flex-col">
              ...
              <LessonCard
                ...
                description="Explore the foundational concepts of cloud computing, including its definition, key features, and how it differs from traditional computing models."
                itemsCount={10}
                index={2}
                isEnrolled={true}
                isAdmin={false}
                itemsCompletedCount={4}
                lessonProgress="in_progress"
              />
            </div>
            <AvgQuizzesPercentScore />
        ...
        <div className="w-full h-full max-w-[480px] bg-white rounded-lg drop-shadow-card p-8 flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-y-1">
            <div className="flex justify-between items-center">
              ...
              <span className="text-neutral-800 body-lg-md px-4">Daily Streak</span>
            </div>
          </div>
        </div>
      </div>
