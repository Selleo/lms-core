...
export class AdminLessonService {
  ...
  async updateQuestionAnswerDisplayOrder(body: {
    questionAnwserId: string;
    displayOrder: number;
  }): Promise<void> {
    const [questionAnswerToUpdate] = await this.adminLessonRepository.getQuestionAnswers(
      body.questionAnwserId,
    );
    ...
    const newDisplayOrder = questionAnswerToUpdate.displayOrder as number;
    ...
    await this.db.transaction(async (trx) => {
      await trx
        ...
        .set({
          displayOrder: sql`CASE
            WHEN ${eq(questionAnswerOptions.id, questionAnswerToUpdate.id)}
              THEN ${newDisplayOrder}
            WHEN ${newDisplayOrder < oldDisplayOrder}
        ...
        .where(eq(questionAnswerOptions.questionId, questionAnswerToUpdate.questionId));
    });
  }
  ...
  async updateQuestionDisplayOrder(body: {
    ...
    const [questionToUpdate] = await this.adminLessonRepository.getQuestionById(body.questionId);
    ...
    const newDisplayOrder = questionToUpdate.displayOrder as number;
    ...
    await this.db.transaction(async (trx) => {
      await trx
        ...
        .set({
          displayOrder: sql`CASE
            WHEN ${eq(questions.id, questionToUpdate.id)}
              THEN ${newDisplayOrder}
            WHEN ${newDisplayOrder < oldDisplayOrder}
        ...
        .where(eq(questions.id, questionToUpdate.id));
    });
  }
